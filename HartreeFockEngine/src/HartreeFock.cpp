...
HartreeFock::HartreeFock(Atom at) {
    atom = Atom(at.numBasisFuncs, at.BasisFuncs, at.nucleiCoords, at.nucleiCharge, at.numElectrons);
...
void HartreeFock::updateDensity() {
    ...
    for (int i = 0; i < atom.numBasisFuncs; i++) {
        for (int j = 0; j < atom.numBasisFuncs; j++) {
            ...
            for (int elec = 0; elec < (atom.numElectrons+1)/2; elec++) {
...
void HartreeFock::SCF(bool logging) {
    ...
    if (logging) {
        std::cout << "As of iteration # " << counter << ": final energy is " << calcEnergyFromDensity() << std::setprecision(10) << " hartrees\n";
    }
...
double HartreeFock::finalSolveEnergy() {
    if (atom.numElectrons == 1) {
        initMatrices();
